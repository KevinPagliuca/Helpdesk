...
module.exports = {
    ...
    async AssignToAdmins(req, res) {
        const user_name = req.headers.user_name;

        const consult = await connection('tickets')
        .select('*')
        .where('status' !== 'Concluído')
        .where('assignTo', user_name);

        if(consult) {
            res.status(200).json(consult);
        } else {
            res.status(404).json({Error: 'Não encontramos nenhum chamado assinado para você!'})
        }
